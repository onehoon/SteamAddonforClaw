...
public sealed class CenterMHelperStagingTests
{
    ...
    public void StageFromPublishRoot_resolves_the_exact_layout_CopyCenterMHelperOutput_writes()
    {
        ...
        try
        {
            ...
            Assert.Equal("MSI Center M.exe", Path.GetFileName(result));
            Assert.Equal(File.ReadAllBytes(sourceBinary), File.ReadAllBytes(result));
        }
        finally
        {
    ...
    public void StageFromPublishRoot_publishRootWithoutCenterMHelperSourceSubfolder_failsClosed()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), "CenterMHelperStagingTests-wrong-" + Guid.NewGuid());
        var wrongRoot = Path.Combine(root, "publish");
        var runtimeDirectory = Path.Combine(root, "runtime");
        ...
        try
        {
            var stagedPath = CenterMHelperStaging.StageFromPublishRoot(wrongRoot, runtimeDirectory);
