...
public sealed class RecoveryManagerTests : IDisposable
{
    ...
    {
        var manager = Manager();
        var result = manager.BeginRecoverySession(Snapshot());
    ...
    {
        var result = new RecoveryManager(new FaultStore(write: true)).BeginRecoverySession(Snapshot());
    ...
    {
        var manager = Manager();
        manager.BeginRecoverySession(Snapshot());
    ...
    {
        var manager = Manager(); manager.BeginRecoverySession(Snapshot());
    ...
    {
        var manager = Manager(); var first = manager.BeginRecoverySession(Snapshot());
        Assert.Equal(RecoveryStatus.Failure, manager.BeginRecoverySession(Snapshot()).Status);
