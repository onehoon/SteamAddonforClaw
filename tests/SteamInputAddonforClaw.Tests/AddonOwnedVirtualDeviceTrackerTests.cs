...
public sealed class AddonOwnedVirtualDeviceTrackerTests
{
    ...
    [Fact]
    public void Gordon_reappearance_of_the_exact_previously_owned_instance_after_absence_blocks_the_clear()
    {
        var tracker = new AddonOwnedVirtualDeviceTracker();
        var ownedDevice = new ControllerDeviceInfo("USB\\VID_28DE&PID_1102\\X", null, UsbIpHostInstanceId, [UsbIpHostInstanceId], "USB", ["HID\\VID_28DE&PID_1102"], [], "HIDClass", null, null, 0x28DE, 0x1102, true);
        tracker.MarkOwnershipUncertain();
        tracker.ResolveOwnership([ownedDevice]);

        // Absence-verification snapshot (not modeled here directly, WaitForAbsenceAsync's concern):
        // the exact owned InstanceId was confirmed gone. The uncertainty-clear snapshot, taken
        // separately/later, shows the SAME exact InstanceId again.
        var uncertaintyClearSnapshot = new[] { UsbIpHost(), ownedDevice };

        var cleared = tracker.ClearUncertaintyAfterVerifiedAbsence(uncertaintyClearSnapshot, new ViiperVirtualDeviceIdentityPolicy(), before: [], owned: [ownedDevice]);

        Assert.False(cleared);
        // The clear must be a no-op on failure: ownership tracking is left exactly as it was
        // (still recorded as owned, not silently reset to "uncertain" or "clear") so the caller's
        // recovery-mutation-remains-incomplete contract has consistent state to reason about.
