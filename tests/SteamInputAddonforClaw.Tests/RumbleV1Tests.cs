...
public sealed class RumbleV1Tests
{
    ...
    {
        var report = new byte[] { 0, 0xEB, 9, 0, 0, 0x34, 0x12, 0xCD, 0xAB, 2, 0 };
    ...
    {
        Assert.Equal(SteamDeckFeedbackCommand.Malformed, SteamDeckRumbleDecoder.Decode([0, 0xEB, 9]).Command);
        Assert.Equal(SteamDeckFeedbackCommand.Unsupported, SteamDeckRumbleDecoder.Decode([0, 0xEA]).Command);
        Assert.Equal(SteamDeckFeedbackCommand.Unsupported, SteamDeckRumbleDecoder.Decode([0, 0x8F]).Command);
        Assert.Equal(SteamDeckFeedbackCommand.Unknown, SteamDeckRumbleDecoder.Decode([0, 0x99]).Command);
    }
