...
public sealed class QamFrontendContractTests
{
    ...
    public void Qam_cpu_boost_panel_uses_the_existing_seven_mode_contract_and_bridge_allowlist()
    {
        ...
        Assert.Contains("const candidates = moduleExports.default", source);
        Assert.Contains("[moduleExports.default, \"default\"], [moduleExports, \"root\"]", source);
        Assert.Contains("modules.push({ exports: candidate, source });", source);
