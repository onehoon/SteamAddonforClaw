...
public sealed class QamFrontendContractTests
{
    ...
    public void Qam_cpu_boost_panel_uses_the_existing_seven_mode_contract_and_bridge_allowlist()
    {
        ...
        Assert.Contains("ToggleField: [\"checked\", \"onChange\"]", source);
        Assert.Contains("SliderField: [\"min\", \"max\", \"step\", \"value\", \"onChange\"]", source);
        ...
        Assert.DoesNotContain("modes.map(([mode])", source);
