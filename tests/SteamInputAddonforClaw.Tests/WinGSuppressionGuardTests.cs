...
public sealed class WinGSuppressionGuardTests
{
    ...
    [Fact] public void PartialCleanupAttemptsDummyUpAndStillSuppresses() { var calls = new List<WinGSuppressionGuard.Input[]>(); using var g = Create(calls, _ => 1); g.EnsureArmed(); g.ProcessKey(0x5B, true); Assert.NotEqual(IntPtr.Zero, g.ProcessKey(0x47, true)); Assert.Equal(2, calls.Count); Assert.Equal((ushort)0xFF, calls[1][0].Data.Vk); Assert.Equal(0x0002u, calls[1][0].Data.Flags); }
