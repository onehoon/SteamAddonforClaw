...
public sealed class NamedPipeAddonFrontendServer : IAsyncDisposable
{
    ...
    private async Task<System.Text.Json.JsonElement> InvokeAsync(FrontendRpcMethod m, System.Text.Json.JsonElement? p, CancellationToken t) => m == FrontendRpcMethod.CaptureAppUpdate
        ...
        : m == FrontendRpcMethod.CaptureSteamFse
        ? FrontendWireCodec.Payload(await _inner.CaptureSteamFseAsync(t).ConfigureAwait(false))
        ...
        : m == FrontendRpcMethod.RequestEnterBios
        ? FrontendWireCodec.Payload(await _inner.RequestEnterBiosAsync(t).ConfigureAwait(false))
