...
// Version 20: Full1902 removed the user-configurable Steam Input Routing preference. The
...
// Version 24: App UI PR-B removes the obsolete LaunchAtWindowsStartup user-preference contract:
// the SetLaunchAtWindowsStartup RPC / SetLaunchAtWindowsStartupRequest / FrontendLaunchAtStartupResult,
...
// CaptureAddonQuickSettingsShell RPC. A v30 peer must fail the handshake up front. Overlay
...
// Version 41: the obsolete Main UI Enter BIOS RPC and its temporary firmware-restart path are removed.
// A v40 peer must fail the handshake before it can request a feature that no longer exists.
// Version 42: the Main App Shortcut editor adds separate capture/mutation and screenshot-folder RPCs.
// A v41 peer must fail the handshake before it can partially understand editor state.
// Version 43: the Developer Vibration Test page adds the Full1902 Xbox360 terminal-STOP loop
// diagnostic capture/start/stop contract. A v42 peer cannot safely use these methods or snapshots.
public static class FrontendTransportProtocol { public const int CurrentVersion = 43; }
...
internal sealed record SetClawHudEnabledRequest(bool Enabled);
internal sealed record SetScreenshotSaveFolderRequest(string? Folder);
