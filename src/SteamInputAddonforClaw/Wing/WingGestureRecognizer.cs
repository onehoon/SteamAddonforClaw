...
internal sealed class WingGestureRecognizer : IDisposable
{
    ...
    internal WingGestureRecognizer(Func<bool> doubleEnabled, IOem1GestureDelay? delay = null)
    { _doubleEnabled = doubleEnabled; _delay = delay ?? new Oem1TaskDelay(); }
    ...
    internal void OnPress()
    {
        ...
        lock (_gate)
        {
            if (_disposed) return;
            second = _first;
            if (second) { _first = false; _generation++; _pending?.Cancel(); _pending = null; }
            else if (_doubleEnabled()) { _first = true; _generation++; _pending = new CancellationTokenSource(); var g = _generation; _ = CompleteAsync(g, _pending.Token); return; }
        }
        Deliver(second ? WingGesture.Double : WingGesture.Single);
    }
