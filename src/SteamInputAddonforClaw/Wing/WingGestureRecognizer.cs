...
internal sealed class WingGestureRecognizer : IDisposable
{
    ...
    internal void OnPress(long authorityEpoch)
    {
        lock (_operationGate)
        {
            ...
            {
                if (_disposed) return;
                if (_first && _firstEpoch == authorityEpoch)
                {
                    var inWindow = _time.GetElapsedTime(_firstTimestamp, now) < DoubleWindow;
                    var epoch = _firstEpoch;
                    _first = false;
                    _generation++;
                    CancelPendingCore();
                    completed = new(inWindow ? WingGesture.Double : WingGesture.Single, epoch);
                    beginNew = !inWindow;
                }
                else
                {
    ...
    private void BeginCurrentPress(long timestamp, long epoch)
    {
        ...
        lock (_gate)
        {
            ...
            _ = CompleteAsync(generation, epoch, _pending.Token);
        }
    }
    ...
    {
        AppLog.Debug("Wing.Event", delivery.Gesture == WingGesture.Single ? "GestureSingle" : "GestureDouble");
