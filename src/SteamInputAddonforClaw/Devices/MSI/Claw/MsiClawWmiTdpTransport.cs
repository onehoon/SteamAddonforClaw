...
internal sealed class MsiClawWmiTdpTransport : IMsiClawTdpTransport
{
    ...
    public bool TrySetData(int block, byte value)
    {
        ...
        data["Bytes"] = package; input["Data"] = data;
    ...
    private static bool TryInvoke(string method, byte[] package, string field, int fieldValue, out byte[] payload)
    {
        ...
        if (output?["Data"] is not ManagementBaseObject response) { LogStage(method, "OutputDataMissing", field, fieldValue); return false; }
        if (response["Bytes"] is not byte[] bytes) { LogStage(method, "OutputBytesMissing", field, fieldValue); return false; }
