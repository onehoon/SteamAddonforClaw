...
internal interface IDisplayResolutionService
{
    bool TryCapture(out DisplayModeSnapshot snapshot);
    bool TryApply(DisplayModeSnapshot current, int width, int height);
    bool TryRestore(DisplayModeSnapshot original);
}

internal sealed class DisplayResolutionService : IDisplayResolutionService
{
    ...
    public bool TryApply(DisplayModeSnapshot current, int width, int height) => Change(current with { Width = width, Height = height }, CdsTest) == 0 && Change(current with { Width = width, Height = height }, 0) == 0;
    public bool TryRestore(DisplayModeSnapshot original) { if (!TryCapture(out var current)) return false; return current == original || (Change(original, CdsTest) == 0 && Change(original, 0) == 0); }
