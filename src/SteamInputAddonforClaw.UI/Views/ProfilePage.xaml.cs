...
public sealed partial class ProfilePage : UserControl
{
    ...
    private bool _active, _suppressEvents, _suppressTdpEvents, _suppressFpsEvents;
    ...
    private CancellationTokenSource? _fpsDebounce; private long _fpsGeneration; private int? _acFpsDraft, _dcFpsDraft;
    ...
    private static readonly ResolutionItem[] ResolutionItems = [new(null, null, "Do not change"), new(1920, 1200, "1920 × 1200"), new(1680, 1050, "1680 × 1050"), new(1440, 900, "1440 × 900"), new(1920, 1080, "1920 × 1080")];
    ...
    private void ClearSelection() { CancelTdpDebounce(); _tdpDraftDirty = false; _selectedGame = null; _snapshot = null; ProfileEnabledToggle.IsOn = false; ProfileEnabledToggle.IsEnabled = false; ResolutionComboBox.SelectedItem = null; ResolutionComboBox.IsEnabled = false; SetEditorsEnabled(false); }
    ...
    private void SetEditorsEnabled(bool enabled) { CpuBoostAcComboBox.IsEnabled = CpuBoostDcComboBox.IsEnabled = enabled; var powerModeEditable = enabled && _snapshot?.PowerMode is not null; PowerModeAcComboBox.IsEnabled = PowerModeDcComboBox.IsEnabled = powerModeEditable; var tdp = enabled && _snapshot?.Limits is not null; foreach (var slider in new[] { AcPl1Slider, AcPl2Slider, DcPl1Slider, DcPl2Slider }) slider.IsEnabled = tdp; var fps = enabled && _snapshot?.FpsLimit?.Available == true; AcFpsSlider.IsEnabled = DcFpsSlider.IsEnabled = fps && FpsEnabledToggle.IsOn; }
    ...
    private void FpsSlider_ValueChanged(object sender, RangeBaseValueChangedEventArgs e) { if (_suppressFpsEvents || _frontend is null || _selectedGame is null) return; var value = Math.Clamp((int)Math.Round(e.NewValue), 40, 120); var ac = ReferenceEquals(sender, AcFpsSlider); if (ac) { _acFpsDraft = value; AcFpsText.Text = $"{value} FPS"; } else { _dcFpsDraft = value; DcFpsText.Text = $"{value} FPS"; } var generation = ++_fpsGeneration; _fpsDebounce?.Cancel(); _fpsDebounce = new(); _ = SubmitFpsAfterDelayAsync(ac, value, generation, _fpsDebounce.Token); }
    private async Task SubmitFpsAfterDelayAsync(bool ac, int value, long generation, CancellationToken token) { try { await Task.Delay(275, token); if (generation != _fpsGeneration || _frontend is null || _selectedGame is null) return; var id = _selectedGame.AppId; var result = ac ? await _frontend.SetGameProfileFpsLimitAcAsync(id, value) : await _frontend.SetGameProfileFpsLimitDcAsync(id, value); if (IsCurrentProfileResponse(_selectedGame?.AppId, id)) { Render(result.Snapshot); if (!result.Succeeded) ShowError(result.FailureMessage ?? "Intel FPS Limit could not be updated.", null); } } catch (OperationCanceledException) { } catch (Exception ex) { if (_selectedGame is not null) await RestoreSelectedAfterMutationFailureAsync(_selectedGame.AppId, "Intel FPS Limit could not be updated.", ex); } }
    private void CancelFpsDebounce() { _fpsGeneration++; _fpsDebounce?.Cancel(); _fpsDebounce = null; }
    ...
    private async void ResolutionComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        var appId = _selectedGame.AppId;
        try { var result = await _frontend.SetGameProfileResolutionAsync(appId, item.Width is { } w && item.Height is { } h ? new(w, h) : null, _selectedGame.Name); if (!IsCurrentProfileResponse(_selectedGame?.AppId, appId)) return; Render(result.Snapshot); if (!result.Succeeded) ShowError(result.FailureMessage ?? "Display resolution could not be updated.", null); }
        catch (Exception exception) { await RestoreSelectedAfterMutationFailureAsync(appId, "Display resolution could not be updated.", exception); }
    }
