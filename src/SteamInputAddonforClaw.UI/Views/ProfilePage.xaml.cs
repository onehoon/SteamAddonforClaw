...
public sealed partial class ProfilePage : UserControl
{
    ...
    private async void RefreshGamesButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (_selectedGame is null) ClearSelection(); else { GameSelector.Text = _selectedGame.Name; await CaptureSelectedAsync(_selectedGame.AppId); } ClearError();
        }
    ...
    private async Task CaptureSelectedAsync(uint appId) { if (_frontend is null) return; try { Render(await _frontend.CaptureGameProfileAsync(appId)); ClearError(); } catch (Exception exception) { ShowError("Profile settings could not be loaded.", exception); } }
    ...
    private async void ProfileEnabledToggle_Toggled(object sender, RoutedEventArgs e)
    {
        ...
        try { var result = await _frontend.SetGameProfileEnabledAsync(_selectedGame.AppId, ProfileEnabledToggle.IsOn, _selectedGame.Name); Render(result.Snapshot); if (result.Succeeded) ClearError(); else ShowError(result.FailureMessage ?? "Profile could not be updated.", null); } catch (Exception exception) { ShowError("Profile could not be updated because the Runtime connection was interrupted.", exception); }
    }
    ...
    private async Task MutateCpuAsync()
    {
        ...
        try { var result = await _frontend.SetGameProfileCpuBoostAsync(_selectedGame.AppId, ac.Mode, dc.Mode); Render(result.Snapshot); if (result.Succeeded) ClearError(); else ShowError(result.FailureMessage ?? "CPU Boost could not be updated.", null); } catch (Exception exception) { ShowError("CPU Boost could not be updated.", exception); }
    }
    ...
    {
        try { await Task.Delay(300, token); if (generation != _tdpGeneration || _frontend is null || _selectedGame is null || _acPl1 is not { } ac1 || _acPl2 is not { } ac2 || _dcPl1 is not { } dc1 || _dcPl2 is not { } dc2) return; var result = await _frontend.SetGameProfileTdpAsync(_selectedGame.AppId, new(new(ac1, ac2), new(dc1, dc2))); Render(result.Snapshot); if (result.Succeeded) ClearError(); else ShowError(result.FailureMessage ?? "TDP could not be updated.", null); } catch (OperationCanceledException) { } catch (Exception exception) { ShowError("TDP could not be updated.", exception); }
    }
