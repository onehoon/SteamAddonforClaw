...
public sealed partial class FanHardwareProbePage : UserControl
{
    ...
    private void SetBusy(bool busy) { CaptureButton.IsEnabled = !busy; AutomaticButton.IsEnabled = !busy; RestoreButton.IsEnabled = !busy; PhysicalButton.IsEnabled = !busy; SuspendButton.IsEnabled = !busy; StatusText.Text = busy ? "Running..." : StatusText.Text; }
    private void Render(FrontendFanProbeSnapshot s) { if (!_active) return; _latest = s; DeviceText.Text = $"Device: {s.Model}"; BoardText.Text = $"Board: {s.BaseBoard}"; ModelText.Text = $"Probe profile: {s.ProbeModel}"; StatusText.Text = $"Status: {s.Status}"; ResultText.Text = $"Result: {s.Status}\nReport: {s.ReportPath ?? "Not generated"}"; ErrorText.Text = s.ErrorMessage ?? ""; ReportButton.IsEnabled = s.HasReport; if (s.Status == "ARMED") { CaptureButton.IsEnabled = false; AutomaticButton.IsEnabled = false; PhysicalButton.IsEnabled = false; SuspendButton.IsEnabled = false; RestoreButton.IsEnabled = true; } }
}
