...
The normal runtime then reads the current RunningAppID, applies the current eligibility checks, and may create a completely new routing session. `RecoveryManager` still records current-process forward mutations (native-state changes, HidHide changes, addon-owned VIIPER output identity) and still backs the existing runtime and power-transition recovery path (`RecoverIncompleteSessionAsync`); it is not the startup target authority and is not involved in stale-journal retirement.
