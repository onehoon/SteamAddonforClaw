...
Previous journal state is not desired current state. Previous routing state is not the current routing decision. At application startup or restart, Stock MSI Center M does not resume or reconstruct a prior routing session, restore its exact native state, replay HidHide changes, or use earlier RunningAppID data. After the independently verified live XInput baseline, `StartupCoordinator` retires any stale previous-process recovery-journal file directly through `IRecoveryJournalStore`: it checks whether the file exists and, if so, deletes it and verifies the deletion. It never reads, deserializes, or otherwise interprets the journal payload, so stale bookkeeping is discarded rather than replayed. If that discard fails, the verified physical controller remains XInput and new routing is passive.

The normal runtime then reads the current RunningAppID, applies the current eligibility checks, and may create a completely new routing session. `RecoveryManager` still records current-process forward mutations (native-state changes, HidHide changes, addon-owned VIIPER output identity) and still backs normal transactional rollback (native-state restore, HidHide entry/whitelist removal, addon-owned virtual-device teardown); it is not the startup target authority and is not involved in stale-journal retirement.

## Power-transition safety

Suspend and hibernate immediately close the forward-mutation gate and invalidate in-flight routing by a power epoch. A narrowly scoped teardown-only window then rolls back the current frozen routing session before sleep; it cannot admit a new routing entry and is sealed after participant processing. On resume, the current process's own canonical routing-pipeline cleanup is authoritative: any residual session or pending cleanup from before suspend is retried first. If a recovery journal still remains after that retried cleanup, it is not replayed -- a remaining journal is fail-closed evidence that cleanup is incomplete or ambiguous, so the addon commits Unsafe and keeps the forward gate closed instead of reconstructing state from it. Only once no journal remains does resume establish the live Stock XInput baseline, open the forward gate, and perform a fresh current-world routing reconciliation. Pre-suspend device handles and virtual-controller state are never trusted or automatically recreated.
...
`RoutingPipelineSessionCoordinator` is the single runtime owner of routing-session state and serializes reconciliation. `ActiveSession == null` means Passive; `ActiveSession != null` means OverrideActive. Current `ControllerManagerClassification` selects the fixed `RoutingPipelinePlan` when a new session enters; once entry succeeds, `ActiveRoutingPipelineSession` freezes only that exact plan for rollback. Repeated eligible observations do not rebuild or replace an active plan, and exit/failure cleanup uses the exact plan frozen at entry. If rollback fails, the active session and frozen plan are preserved so cleanup can be retried. Pending cleanup records incomplete rollback work but does not create another operational-state authority. Plan selection is not routing authorization.
